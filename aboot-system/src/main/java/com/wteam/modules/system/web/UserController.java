...
public class UserController {
    ...
    public R getUsers(UserQueryCriteria criteria, Pageable pageable){
        ...
        if (!CollectionUtils.isEmpty(deptIds) && !CollectionUtils.isEmpty(deptSet)){
            ...
            //否则取并集
        }else {//这里else包括三种情况，一种是部门查询条件为空，则result.addAll(deptSet)
            //另一种是数据权限为空，则result.addAll(depIds)
            //最后一种是都为空。结合一下两个都addAll。
            result.addAll(deptSet);
     ...
     * 就是不能赋予其他用户比自己高的角色，例如普通管理员不能赋予别人超级管理员的身份
     * @param resources /
     */
